...
Start-Process -WorkingDirectory $backendRoot -FilePath "powershell.exe" `
    -ArgumentList "-NoExit", "-Command", "& '$venvPython' -m uvicorn app.main:app --reload --port 7000"

Start-Process -WorkingDirectory $backendRoot -FilePath "powershell.exe" `
    -ArgumentList "-NoExit", "-Command", "& '$venvPython' -m arq app.workers.queue.WorkerSettings"

Write-Host "Started API (http://127.0.0.1:7000) and ARQ worker in separate windows. Close either window to stop it."
